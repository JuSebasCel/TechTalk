...
void setup() {
  ...
  //"DEBUG" Ingreso de eje Z de manera manual.
  Serial.println("Introduce valor de Z manual vía consola: escribe un número y presiona ENTER");
}
...
void loop() {
  ...
  // Lectura de valor manual de Z por Serial
  if (Serial.available()) {
    int valor = Serial.parseInt();
    if (valor != 0 || Serial.read() == '\n') {
      zManual = valor;
      Serial.print("Valor Z manual actualizado a: ");
      Serial.println(zManual);
    }
  }
